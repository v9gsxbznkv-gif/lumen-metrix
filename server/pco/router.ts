...
export const pcoRouter = router({
  ...
  getDashboardData: publicProcedure.query(async () => {
    ...
    return {
      ...
      next_steps_weekly: nextStepsWeeklyRows.map((r) => ({
        year: r.year,
        weekNumber: r.weekNumber,
        weekStartDate: r.weekStartDate,
        campus: r.campus,
      ...
      serving_weekly: servingWeeklyRows.map((r) => ({
        year: r.year,
        weekNumber: r.weekNumber,
        weekStartDate: r.weekStartDate,
        campus: r.campus,
  ...
  /** Debug: Dump all location names from PCO Check-Ins for Canton/Jasper events */
  debugCheckInLocations: publicProcedure
    .query(async () => {
      const client = await createAuthenticatedPcoClient();
      if (!client) throw new Error('Not connected to PCO');

      // Get all check-in events
      const eventsResult = await client.paginateAll('/check-ins/v2/events', { per_page: 50 });
      const cantonEvent = (eventsResult.data as any[]).find((e: any) => e.attributes.name === 'Revolution Canton Check-In');
      const jasperEvent = (eventsResult.data as any[]).find((e: any) => e.attributes.name === 'Revolution Jasper Check-In');

      const results: { event: string; period: string; locations: { name: string; regular: number; guest: number; total: number }[] }[] = [];

      for (const ev of [cantonEvent, jasperEvent].filter(Boolean)) {
        const eventName = ev.attributes.name;
        // Get most recent 2 event periods
        const periodsResult = await client.paginateAll(
          `/check-ins/v2/events/${ev.id}/event_periods`,
          { per_page: 2, order: '-starts_at' }
        );

        for (const period of (periodsResult.data as any[]).slice(0, 1)) {
          // Get event_times for this period
          const timesResult = await client.paginateAll(
            `/check-ins/v2/event_periods/${period.id}/event_times`,
            { per_page: 50 }
          );

          const allLocations: { name: string; regular: number; guest: number; total: number }[] = [];

          for (const et of (timesResult.data as any[]).slice(0, 3)) {
            try {
              const locResult = await Promise.race([
                client.paginateAll(
                  `/check-ins/v2/event_times/${et.id}/location_event_times`,
              ...
              for (const let_ of locResult.data as any[]) {
                ...
                allLocations.push({ name: locName || '(unknown)', regular, guest, total: regular + guest });
              }
            } catch (err: any) {
              allLocations.push({ name: `ERROR: ${err.message}`, regular: 0, guest: 0, total: 0 });
            }
          }

          results.push({
            event: eventName,
            period: period.attributes.starts_at,
